...
public class DisplayView extends Canvas implements Observer
{
  ...
  private BufferedImage image, wait, pick, collect;
  ...
  public void drawActualScreen( Graphics2D g )  // Re draw contents 
  {
    ...
    g.fill( new Rectangle2D.Double( 0, 80, W, 325));
    g.setPaint( Color.WHITE);
    g.fill( new Rectangle2D.Double( 260, 107, 215, 235));
    try {
		image = ImageIO.read(new File("images/logo.png"));
		wait = ImageIO.read(new File("images/waiting.png"));
		pick = ImageIO.read(new File("images/pick.png"));
		collect = ImageIO.read(new File("images/collect.png"));
	} catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
    int x = (W - image.getWidth(null))/2;
    g.drawImage(image, x, 2, this); // see javadoc for more info on the parameters
 
    g.drawImage(wait, 0, 107, this);
    g.drawImage(pick, 0, 207, this);
    g.drawImage(collect, 0, 307, this);
    // Draw state of system on display
    String lines[] = textToDisplay.split("\n");
    g.setPaint( Color.black );
    for ( int i=0; i<lines.length; i++ )
    {
      g.drawString( lines[i], 0, 80 + 50*i );
    }
